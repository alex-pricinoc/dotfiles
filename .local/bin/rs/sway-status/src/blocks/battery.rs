...
impl Battery {
    fn new() -> Self {
        ...
        let (capacity, power_now, current_now, voltage_now) = (
            Self::read_prop::<f64>(battery_path, "capacity"),
            Self::read_prop::<f64>(battery_path, "power_now"), // uW
            Self::read_prop::<f64>(battery_path, "current_now"), // uA
            Self::read_prop::<f64>(battery_path, "voltage_now"), // uV
        );
        ...
        let ac = Self::read_prop::<u8>(power_supply_path, "online").is_some_and(|x| x == 1);
        let capacity = capacity.unwrap_or_default();
        let power = power_now.unwrap_or_default();
